...
class Array
  def bubble_sort!(&prc)
    ...
    while !sorted
      ...
      (0...self.length - 1) do |i|
        if prc.call(self[i], self[i + 1]) == 1 #only swap if i > i+1
  ...
  #does the same but doesn't modify the original
  def bubble_sort(&prc)
